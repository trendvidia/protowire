...
- ✅ **`bench-pxf-android` + `bench-sbe-android` lite-tier perf harnesses.** Cross-port `scripts/cross_pxf_bench.sh` and `scripts/cross_sbe_bench.sh` report `java-lite` rows alongside `java`. On the canonical 94-byte SBE fixture, lite is 2.4× / 4.1× faster than the descriptor-driven path on marshal / unmarshal.
  ...
  **Left out of the public release, then brought back.** The `*-runtime`, `*-android` and `bench-*-android` items above landed on the pre-public protowire-java tree (its PRs #13–#25, 2026-05-04/05) and were left out of the 2026-05-06 "Initial public release" root, whose commit lists the modules it ships and has none of them; no tag up to v1.0.1 carries them (measured 2026-09-06). They return to `main` through the PRs on [trendvidia/protowire-java#60](https://github.com/trendvidia/protowire-java/issues/60), redone against today's parser (the `@dataset` / `@proto` grammar and the registered extension numbers had moved underneath them), and the two lite dumpers gained the gate's `--pb` / `--sbe` fixture legs on the way ([protowire-java#69](https://github.com/trendvidia/protowire-java/pull/69)). The cross-port scripts' `java-lite` rows run against any checkout that has the modules and skip otherwise.
