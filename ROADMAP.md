...
- ✅ **`bench-pxf-android` + `bench-sbe-android` lite-tier perf harnesses.** Cross-port `scripts/cross_pxf_bench.sh` and `scripts/cross_sbe_bench.sh` report `java-lite` rows alongside `java`. On the canonical 94-byte SBE fixture, lite is 2.4× / 4.1× faster than the descriptor-driven path on marshal / unmarshal.
  ...
  **Not in the public release.** The `*-runtime`, `*-android` and `bench-*-android` items above landed on the pre-public protowire-java tree (its PRs #13–#25, 2026-05-04/05) and were left out of the 2026-05-06 "Initial public release" root, whose commit lists the modules it ships — `pb`, `pxf`, `sbe`, `envelope`, `proto-annotations` and the three harnesses — and has none of them; no tag since carries them (measured 2026-09-06). The cross-port scripts skip their `java-lite` rows when the modules are absent, which is always, on a public checkout. Whether the lite tier returns is open in [trendvidia/protowire-java#58](https://github.com/trendvidia/protowire-java/issues/58); the 0.73.0 and 0.75.0 items below inherit that question.
