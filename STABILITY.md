...
v1.11 states, for the first time, where `(pxf.default)` and `@default(value)` may be placed (GH [#223](https://github.com/trendvidia/protowire/issues/223); draft `-01` §annotation-extensions, "Default Placement"), and what both `pxf.default` and `pxf.required` mean on a member of a oneof (GH [#226](https://github.com/trendvidia/protowire/issues/226); draft `-01` §annotation-extensions, "Oneof Members"). No grammar, wire-format, envelope, report, or descriptor change: PXF, `pb`, SBE, envelope and report outputs are byte-identical to v1.10 for every schema and document that binds under both **except** for the oneof case called out below, which is a deliberate behaviour change.
