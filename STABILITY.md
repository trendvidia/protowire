...
**Lite-mode emitted code is wire-equivalent to full-mode for the same `.proto` input.** The mechanism meant to enforce it is [`scripts/cross_envelope_check.sh`](scripts/cross_envelope_check.sh)'s `java-lite` / `java-pxf-lite` rows, which fail the run when a lite row's bytes differ from the JVM `java` row. As of v1.12 nothing enforces it: the `dump-envelope-android` and `dump-envelope-pxf-android` modules those rows build have never been merged to protowire-java `main` (they exist on its `lite-encoder-bignum` and `sbe-codegen-it-composite-fixture` branches), CI runs the script with the lite rows opted out, and a local run skips them when the modules are absent. Until they land, this paragraph is a statement of intent, not a promise — the same correction promise 2 received in v1.12 for Dart ([trendvidia/protowire-java#58](https://github.com/trendvidia/protowire-java/issues/58)).
