...
### v1.11 — `pxf.default` placement constraint

v1.11 states, for the first time, where `(pxf.default)` and `@default(value)` may be placed (GH [#223](https://github.com/trendvidia/protowire/issues/223); draft `-01` §annotation-extensions, "Default Placement"). No grammar, wire-format, envelope, report, or descriptor change: PXF, `pb`, SBE, envelope and report outputs are byte-identical to v1.10 for every schema and document that binds under both.
