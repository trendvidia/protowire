...
Targets that strip protobuf descriptor reflection at runtime — the lite tier — drop a documented set of capabilities relative to the full tier. This was written for the Java port's `*-android` modules built on `protobuf-javalite`, which are not part of the public protowire-java release (below); the same exclusions apply to any future `*-lite` target in any port.
...
**Lite-mode emitted code is wire-equivalent to full-mode for the same `.proto` input.** The mechanism meant to enforce it is [`scripts/cross_envelope_check.sh`](scripts/cross_envelope_check.sh)'s `java-lite` / `java-pxf-lite` rows, which fail the run when a lite row's bytes differ from the JVM `java` row. As of v1.12 nothing enforces it, because the modules those rows build are not in the public protowire-java repository at all: the lite tier (`pxf-android`, `envelope-android`, `sbe-android`, `pb-android`, the `*-android` dumpers and benches, and the `pxf-runtime` / `sbe-runtime` extractions they depend on) landed on protowire-java's pre-public tree in 2026-05 and was left out of the 2026-05-06 public-release root, whose commit lists the modules it ships; no tag since carries it. CI runs the script with the lite rows opted out, and a local run skips them when the modules are absent, which on a public checkout is always. Until the tier is either brought back or declared out of scope ([trendvidia/protowire-java#58](https://github.com/trendvidia/protowire-java/issues/58)), this paragraph is a statement of intent, not a promise — the same correction promise 2 received in v1.12 for Dart.
