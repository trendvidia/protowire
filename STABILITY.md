...
2. **PB and SBE wire formats.** The byte-level layouts that Go's reference implementation produces and accepts at `0.73.0` are the contract. Other ports must round-trip those bytes unchanged. This is enforced by [`scripts/cross_envelope_check.sh`](scripts/cross_envelope_check.sh), run by the [`cross-port-envelope-check`](.github/workflows/cross-port-envelope-check.yml) workflow on PRs touching `proto/**` or `scripts/cross_*`, on a daily schedule, and on demand (see [M1 in `ROADMAP.md`](ROADMAP.md#m1--ci-gating-target-0710)). **That gate covers Go, C++, TypeScript and Java.** Rust, Swift and C# are exercised by the same script but only when it is run locally — for those, promise 2 rests on a pre-release manual run, not on CI. The Java/Android lite rows (`java-lite`, `java-pxf-lite`) run only against a protowire-java checkout that has the `*-android` modules; the script skips them when they are absent and says so. Those modules were left out of protowire-java's public release and are back on its `main` as of 2026-09-06 ([trendvidia/protowire-java#60](https://github.com/trendvidia/protowire-java/issues/60); see the runtime-tier section). Dart's `dump_envelope` exists from v1.12 ([trendvidia/protowire-dart#15](https://github.com/trendvidia/protowire-dart/pull/15)); before it, the script aborted at the Dart step, which is why it now skips a port whose dumper is absent and says so. The daily schedule exists because the check reads every port at its default branch: a regression pushed to a port is otherwise invisible here until someone touches this repo. Since v1.12 the same run also hands each descriptor-driven port a descriptor set and a PXF document from [`testdata/`](testdata/) through `dump-envelope --pb` / `--sbe` and compares the bytes with a checked-in golden; that is what enforces promise 3 at runtime rather than by inspecting sources (see the v1.12 section).
3. **Annotation extension field numbers.** The integers in [`proto/pxf/annotations.proto`](proto/pxf/annotations.proto), [`proto/sbe/annotations.proto`](proto/sbe/annotations.proto) and [`proto/schema/v1/descriptor.proto`](proto/schema/v1/descriptor.proto) — `(pxf.required) = 1314`, `(pxf.default) = 1315`, `(sbe.template_id) = 1321`, the `1327` annotation carrier, etc. — are baked into every emitted descriptor and every cross-port codec. Renumbering them breaks every previously-generated `<Message>PxfMeta` and every wire-encoded `FileOptions`/`MessageOptions`/`FieldOptions` blob in the wild. **Do not change them.** New extensions may be added with new numbers inside protowire's registered block, `1314`–`1363`.

   **This promise re-arms at v1.12.0, the first tagged release after 2026-08-31, and is stated here in its amended form because it was itself broken once, deliberately.** Until v1.12.0 the numbers lived in an unregistered `50000`–`59999` range that this project had simply squatted; they moved to `1314`–`1363` — the block the global extension registry granted protowire in [protocolbuffers/protobuf#28919](https://github.com/protocolbuffers/protobuf/pull/28919) — in a single coordinated change across every repo that reads them ([#244](https://github.com/trendvidia/protowire/issues/244)).

   That move contradicted this promise as written. It was permissible for one reason, which will not be available again: the promise's own premise — that the numbers are baked into blobs *in the wild* — was **false**, because the family had no deployed consumers on 2026-08-31. The premise was checked rather than assumed before the change was taken ([#242](https://github.com/trendvidia/protowire/issues/242), option A). Once this promise re-arms, a renumber requires a major version bump, and the argument used in 2026 is spent.

   The retired ranges are **dead and must never be reused**: `50000`–`50002` (PXF), `50100`–`50301` (SBE), `50400`–`50404` (schema-extension carriers), `51000`–`51002` (protocheck), and the earlier `50100`–`50104` carrier draft that was retired pre-v1.2 for colliding with SBE. A descriptor carrying any of them predates v1.12.0 and should be diagnosed as stale rather than silently misread — the surfaces are disjoint, and no port reads both.

   Allocation within the registered block:
   ...
   | `1327`–`1346` | Schema-extension carriers | `1327` `*_annotations` (shared across all 8 `*Options`), `1328` `functions`, `1329` `annotation_decls`, `1330` `type_decls`, `1331` `source_map` |
   | `1347`–`1354` | Validation constraints (protocheck) | `1347` `field`, `1348` `message`, `1349` `oneof` |
   | `1355`–`1359` | OpenAPI generation | none yet |
   | `1360`–`1363` | Unallocated reserve | — |

   Numbers protowire does **not** own are out of scope for this promise and are governed by their owners — `google.api.http` (`72295728`) is written by `@http` lowering and is googleapis', not ours.
...
- **Backward compatibility with stock tooling.** The carrier extensions at `50400`–`50404` (`1327`–`1331` since v1.12) are well-formed proto3. Stock `protoc`, `protobuf-go`, and every existing protowire port treat them as opaque options when `proto/schema/v1/descriptor.proto` is not imported, preserving them byte-identically across decode/re-encode. Tools that opt into the extensions decode them as typed values.
...
- **Extension number claimed.** `(pxf.key) = 50002` (`1316` since v1.12) on `FieldOptions` in [`proto/pxf/annotations.proto`](proto/pxf/annotations.proto). The renumbering prohibition of promise 3 applies.
...
- **Descriptor output changes for every image, not only for `@http` schemas.** Methods carrying `@http` gain `MethodOptions` field `72295728` (`google.api.HttpRule`) beside the `50400` (`1327` since v1.12) carrier (GH [#213](https://github.com/trendvidia/protowire/issues/213); the number is governed by googleapis, not by this document — see the v1.2 bullet on the one foreign extension emitted). Independently, the bundled `protowire/schema/v1/annotations.proto` grew a doc comment, so the `50402` (`1329` since v1.12) source spans it records shifted; that library ships inside every image, so a schema using no `@http` at all still rebuilds to different bytes and every downstream image cache invalidates. `pxf build --google-api-http=false` suppresses the standard option, not the span shift.
...
### v1.12 — extension numbers move to the registered block

v1.12 moves every extension number the family owns out of the unregistered `50000`–`59999` range and into protowire's registered block, `1314`–`1363` (GH [#244](https://github.com/trendvidia/protowire/issues/244); decision [#242](https://github.com/trendvidia/protowire/issues/242)). Promise 3 above carries the allocation table and is stated in its amended form; this section records the release-level consequences in the style of the v1.0 one-time changes, because this is a break the promise forbids and the reasoning should be findable where the other deliberate breaks are.

- **A descriptor-contract break, taken at a minor because its premise was measured false.** Promise 3 forbids renumbering on the premise that the numbers are baked into blobs *in the wild*. On 2026-08-29 there were no deployed consumers — the only public artifact, `@trendvidia/protowire@1.2.0` on npm, drew mirror-and-CI traffic, and every other reader is in this organisation and moved in the same train — so the break cost nobody a migration. That argument is spent: the promise re-arms at the first tag after this change, and from then on a renumber is a major bump.
- **PXF text, `pb`, SBE and envelope bytes are unchanged.** The numbers live in descriptors, not in encoded payloads. `scripts/cross_envelope_check.sh` shows Go, C++, TypeScript and Java byte-identical before and after, and the 94 SBE bytes of [`testdata/sbe-bench.pxf`](testdata/sbe-bench.pxf) are the same bytes they were: the header carries the *values* of `schema_id` and `template_id`, not the extension numbers they were read from.
- **Reading a pre-v1.12 descriptor is not supported.** There is no dual-read: a v1.12 reader looks only at the registered numbers, so a descriptor compiled before the move loses every annotation it carried — `(pxf.required)` silently stops being enforced, `(pxf.default)` stops applying, and an SBE file is reported as having no `(sbe.schema_id)`. Recompile it against the matching `annotations.proto`. The retired ranges named in promise 3 are dead so that such a descriptor is diagnosable rather than merely unknown; draft `-01` says an implementation SHOULD diagnose it, and [trendvidia/protowire-go#98](https://github.com/trendvidia/protowire-go/issues/98) tracks doing so in the reference binder. One port, Rust, resolves the SBE options by extension name from the declarations inside the descriptor set rather than by number, and so reads a self-consistent pre-v1.12 set correctly; that is not a promise, and a set compiled without `--include_imports` gives it nothing to resolve against.
- **Compiled fixtures went stale, and nothing noticed.** [`testdata/sbe-bench.binpb`](testdata/sbe-bench.binpb) and [`testdata/adversarial/adversarial.binpb`](testdata/adversarial/adversarial.binpb) kept `50100`/`50101`/`50200`/`50300` after every reader had moved to `1319`–`1322`, because nothing regenerates a `.binpb` when the `annotations.proto` it imports changes. Every descriptor-driven port's `dump-envelope --sbe` fails on the shared fixture with `missing (sbe.schema_id) option` (Go, C++, TypeScript, Java; protowire-go v1.5.1 fails, v1.4.1 passes), which means `scripts/cross_sbe_bench.sh` had been broken for every port since the renumber. Both sets are regenerated, and [`cmd/pxf/fixture_numbers_test.go`](cmd/pxf/fixture_numbers_test.go) fails on any descriptor set under `testdata/` that carries a retired number, lacks the numbers it must carry, or is not classified there.
- **The gate proves the numbers, not only the envelope.** `cross_envelope_check.sh` gains two legs: every descriptor-driven port's `dump-envelope` decodes the same `testdata/` descriptor set and PXF document through `--pb` and `--sbe`, and the bytes are compared with a checked-in golden — a golden, because ports that all misread the same number would agree with each other. [`testdata/annotations/`](testdata/annotations/) proves `1314` and `1315` (a document whose absent fields take their `(pxf.default)`, and one whose absent `(pxf.required)` field must be rejected); `sbe-bench.pxf` proves `1319`–`1322`. Against a descriptor compiled with the retired numbers, every by-number port emits the document without its defaults and accepts the one it must reject — the failure class both near-misses on #244 belong to, now caught mechanically. The legs cover Go, C++, TypeScript and Java in CI and Rust, Swift and Dart locally. Swift and Dart generate their message types ahead of time, so their dumpers name a generated type and read the ids from the descriptor set — through `SBE.templates(from:)` and `Codec.registerFromDescriptorSet`, both added for this ([protowire-swift#12](https://github.com/trendvidia/protowire-swift/pull/12), [protowire-dart#16](https://github.com/trendvidia/protowire-dart/pull/16)); their pb leg is a **declared** omission, because neither port's PXF decoder implements `(pxf.required)` or `(pxf.default)` at all ([protowire-swift#11](https://github.com/trendvidia/protowire-swift/issues/11), [protowire-dart#14](https://github.com/trendvidia/protowire-dart/issues/14)), and the script fails on an undeclared one. (Both became real after v1.12.0 — [protowire-swift#13](https://github.com/trendvidia/protowire-swift/pull/13) applies both annotations from the descriptor set on the Codable path, [protowire-dart#20](https://github.com/trendvidia/protowire-dart/pull/20) through `PxfAnnotations` on `unmarshalFull` — so no port's leg is declared now.) C# and the Java/Android lite modules were still `SKIP` lines citing [protowire-csharp#26](https://github.com/trendvidia/protowire-csharp/issues/26) and [protowire-java#58](https://github.com/trendvidia/protowire-java/issues/58) at v1.12.0; both have since become real rows (protowire-java#69, [protowire-csharp#28](https://github.com/trendvidia/protowire-csharp/pull/28)), so the gate carries no `SKIP` line at all now. Python inherits protowire-cpp's numbers at wheel-build time and is covered by C++.
- **Carrier headroom shrinks.** The v1.2 text reserved `50405`–`50499` — 95 numbers — for future schema-extension carriers; the whole registered block is 50. Carriers get `1327`–`1346`, 20 numbers, weighted above the other families because RFC-001 is the one actively growing. The `50405`–`50499` reservation is void.

**Also in v1.12 — arbitrary-precision annotation arguments.** The section
below was drafted as v1.13 while the renumber was landing; the renumber's
close-out (stale fixtures, the annotation legs of the gate, this text)
merged after it, so no commit carries one without the other and they ship
as one release.
...
that uses none of them is byte-identical to v1.11 in this respect (the
renumber above still applies to every annotation-bearing descriptor).
...
Targets that strip protobuf descriptor reflection at runtime — the lite tier — drop a documented set of capabilities relative to the full tier. This was written for the Java port's `*-android` modules built on `protobuf-javalite`; the same exclusions apply to any future `*-lite` target in any port.
...
| Wire equivalence with the full tier | n/a | ✓ — intended; see below for what enforces it |

**Lite-mode emitted code is wire-equivalent to full-mode for the same `.proto` input.** The mechanism meant to enforce it is [`scripts/cross_envelope_check.sh`](scripts/cross_envelope_check.sh)'s `java-lite` / `java-pxf-lite` rows, which fail the run when a lite row's bytes differ from the JVM `java` row. The modules those rows build — the lite tier: `pxf-android`, `envelope-android`, `sbe-android`, `pb-android`, the `*-android` dumpers and benches, and the `pxf-runtime` / `sbe-runtime` extractions they depend on — landed on protowire-java's pre-public tree in 2026-05 and were left out of the 2026-05-06 public-release root; no tag up to v1.0.1 carries them. They are back on protowire-java `main` as of 2026-09-06, redone against today's parser (protowire-java#61, #65, #66, #68, #69 under [#60](https://github.com/trendvidia/protowire-java/issues/60)), and both lite dumpers take the `--pb` / `--sbe` fixture legs of the gate: the first run of those legs found that the lite writer applied string defaults differently from the JVM decoder, which is the class of divergence this paragraph exists to forbid, and it was fixed before landing. CI still runs the script with the lite rows opted out; the rows run locally against a `main` checkout, and the run recorded on protowire#270 shows both lite rows byte-identical to the JVM row. The `protowire-*-android` and `protowire-*-runtime` Maven coordinates appear with the next protowire-java release, which is what publishes them.
