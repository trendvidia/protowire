...
The two jobs `MaxNumericLiteralDigits` does are independent, and neither implies the other: the scale cap is a *work* bound on the PB decoder, the digit cap a *length* bound on text. A big number can therefore be representable on the wire and have no PXF spelling, and this is not a boundary one-off. `BigInt.abs` and `Decimal.unscaled` are bounded on the wire only by `MaxMessageSize`, so a `BigInt` of 5000 digits, or a `Decimal` with 5000 unscaled digits at scale 0, is accepted over PB and has no literal under the cap; a `Decimal` at exactly scale ±4096 is the smallest instance (a positive scale of *n* renders *n*+1 positional digits, the leading `0` included, so every value at the scale cap renders to 4097 or more). A marshaller or binder that meets such a value **must** fail with an error naming the digit count, as it already must for a `BigFloat`; it must not truncate, round or switch notation. The `pb/decimal-scale-4096.binpb` corpus row stays `accept` (issue #310, decided as option 1: moving the scale cap by one would make the boundary pair renderable and leave every other wire-only value where it is, for a MUST-level acceptance change in every port; a grammar change for exponent-form Decimal literals would be the same non-result at a higher cost).
