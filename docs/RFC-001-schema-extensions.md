...
A REST-surface renderer normalises only what its own format cannot express: OpenAPI has no `{name=pattern}` template form, so the **path key** drops the constraint and keeps the variable (`/named/{name}`), while a dotted variable carries through unchanged because OpenAPI parameter names admit dots (`/shelves/{shelf.id}`). One consequence is recorded rather than hidden: a nested binding takes its whole top-level container out of the remaining-field binding, so a sibling of a bound leaf is documented nowhere — the reference renderer flattens no message anywhere, and introducing that scheme for this one case needs decisions tracked in issue #218. Conformance fixture: `testdata/schema-extensions/23_http_template_paths.proto`.
