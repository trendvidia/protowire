...
plus custom-option carriers in extension numbers `1327`–`1331`
...
- **Extension number range.** Reservation is `1327`–`1346` for schema-extension carriers. `1327`–`1331` are allocated (the `1319`–`1320` block is skipped because SBE already uses it on `FileOptions`). Allocations beyond this range MUST go through an RFC update; renumbering is a wire break.
