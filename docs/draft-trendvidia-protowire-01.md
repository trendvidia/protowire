...
Canonical spelling of string keys. Because the bare spellings true, false and 123 are a bool key, a bool key and an integer key rather than the strings they resemble, the quotes on a string key are meaningful, and a formatter MUST NOT drop them where the bare form would parse as something else. A formatter or marshaller writes a string key of a map\<K,V\> literal unquoted whenever the key is identifier-safe — it matches the *identifier* production of {{abnf-grammar}} and is not one of the value keywords "null", "true", "false" — and quoted otherwise; in particular the keys "true", "false" and "null", and any key matching the *integer* production, are always written quoted. A quoted identifier-safe key MUST be canonicalized to its unquoted spelling. This is the rule {{keyed-encoding}} states for keyed entry names, applied to map keys; the reference marshaller has always written keys this way, and the formatter now follows it (issue #306). The fixture pair "fmt-keyword-keys" under testdata/map-keys/ pins it.
