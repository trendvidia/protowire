...
declarations (type, function, annotation), a unified annotation
use-site syntax, and a structured validation error model. The
additions are strictly additive; every schema valid under -00
(corresponding to protowire v1.0-v1.1) remains valid under -01. The
substantive content of this revision is concentrated in the new
Schema Language Extensions section and additions to the Annotation
Extensions section and the ABNF appendix; other sections are
materially unchanged from -00 and are reproduced verbatim. The
corresponding design-rationale document, RFC-001, was ratified on
2026-07-16 and is published in the protowire source repository.
...
Canonical spelling of map keys. Because the bare spellings true, false and 123 are a bool key, a bool key and an integer key rather than the strings they resemble, the quotes on a key are meaningful, and a formatter MUST NOT add or drop them where doing so would change what the key denotes. A formatter writes a key bare when the document wrote it bare. A quoted key is written bare when it is identifier-safe — it matches the *identifier* production of {{abnf-grammar}} and is not one of the value keywords "null", "true", "false" — and quoted otherwise; in particular a quoted "true", "false" or "null", and a quoted key matching the *integer* production, stay quoted. Formatters therefore retain whether a map key was written quoted, as {{keyed-encoding}} already requires of keyed entry names. A marshaller, which has no document spelling to preserve, writes a string key bare under the same identifier-safe test and quoted otherwise, and writes bool and integer keys bare; a formatter and a marshaller thus agree on every key that both can produce (issue #306). The fixture pairs "fmt-keyword-keys" and "fmt-bare-keys" under testdata/map-keys/ pin both directions.
...
| `sensitive` | (none) | field, type, message |
...
{{cross-runtime}}. The signature alone is the contract between
...
~~~
annotation-decl = "annotation" ws name ws "("
                  [ annot-param-list ] ")" ";"
...
{{abnf-additions}}): a list literal (`["US", "CA", "GB"]`), a message
...
1. For a well-known wrapper type (`google.protobuf.StringValue` etc.),
   `this` binds to the **unwrapped** scalar value when the wrapper is
   set; the rule is NOT evaluated when the wrapper is null.

2. For `google.protobuf.Timestamp` and `google.protobuf.Duration`,
...
depth 0; each message-typed value entered adds 1. The normative
default limit is **64**, configurable per project via the
`max_recursion_depth` field of the engine configuration
({{engine-config}}); a configured value of 0 means the default.

At the limit the engine neither errors nor silently truncates: it
emits a fail-closed synthetic Violation with code
`protowire.depth_exceeded` for the subtree it did not enter, sets
`truncated = true` on the Report, and — in collect-all mode —
continues with the siblings. The depth definition, the default, and
the at-limit behavior are normative; the enforcement mechanism is
implementation-defined.

The `protowire.` violation-code namespace is reserved for
specification-defined codes; user schemas and engines MUST NOT mint
codes in it.

## Streaming RPC Validation {#streaming-validation}

Validation over streaming RPCs is **per-message**: each message is
validated independently as it crosses the wire, under the same rules
as unary payloads. The receiver MUST validate each message before
delivering it to application code; the sender MAY additionally
validate before transmission. There is no stream-scoped rule surface
in this revision (aggregate invariants across a stream's messages are
an open issue; see {{open-issues}}).

The first invalid message terminates the stream with an error carrying
the Report; there is no lenient mode and no rollback of
previously-delivered messages. Status mapping is direction-asymmetric.
For gRPC transports, the reference mapping is: a request-direction
violation maps to `INVALID_ARGUMENT`; a server-produced response
caught by the sender's pre-send check maps to `INTERNAL`; a client-side
receiver check surfaces as a local error without a wire status. The
Report travels in `google.rpc.Status.details` as a
`google.protobuf.Any`.

## Sensitivity Classification {#sensitivity}

The framework annotation `sensitive` ({{schema-extension-annotations}})
marks a value as sensitive. It is **classification, not protection**:
wire encoding, storage, programmatic access, and validation semantics
are unaffected. How a sensitive value is protected (encryption, key
management) is out of scope for the schema layer.

`@sensitive` attaches to fields, to type declarations (where it
macro-expands to every consuming field, like any other type-attached
annotation), and to messages (where it applies transitively through
message-typed fields).

Consumers MUST observe the following minima:

* Rendering surfaces (editors, log formatters, diagnostic output)
  MUST redact sensitive values, rendering the placeholder `[REDACTED]`
  in place of the value.

* Validation engines MUST withhold the offending value from emitted
  Reports: `actual_value` is left absent and `value_redacted` is set
  ({{error-model}}), keeping redaction distinguishable from the
  three-state absence semantics.

* Documentation generators MUST NOT emit values or examples for
  sensitive declarations; an `@example` on a sensitive declaration is
  a compile-time warning.

`@sensitive` lowers through the standard annotation carrier
({{lowering}}); it has no dedicated descriptor surface.
...
| Field | Type | Description |
|---|---|---|
| `path` | field path | Structured path into the message instance: a segment list carrying field name, field number, and — where applicable — one typed subscript (repeated index, or a map key in its declared key type). The path is never a dotted string. |
...
The on-wire shape of the aggregate Report is specified normatively by
`protowire/proto/schema/v1/report.proto` in the protowire source
repository. It is a runtime artifact — plain proto3 messages carrying
no descriptor extension numbers — so all ports emit interchangeable
reports across PXF, PB, and the envelope. Summary of the shape:

* `Report` carries the validated `message_type`, the list of
  `EnrichedViolation`s, the execution `mode` (collect-all or
  fail-fast), the `truncated` flag ({{recursion-depth}}), engine
  name/version, and wall-clock evaluation time.

* `EnrichedViolation` carries the causal `Violation` (`code`,
...
* Values (`actual_value` and `params` entries) use a dedicated typed
  ...
  enum-value name, packed message, or list — deliberately **not**
  `google.protobuf.Struct`'s value type, which folds integers to
  double, cannot carry bytes, and erases the distinction between null
  and absent.
...
Project-level engine selection lives in a text-format Protocol Buffers
...
Discovery is nearest-config-wins, walking from the compilation's
...
The two bounds MaxNumericLiteralDigits states are independent: the scale bound limits the work a PB decoder performs, the digit bound limits the length of a text literal, and neither implies the other. A big number MAY therefore be representable on the wire and have no PXF spelling: BigInt.abs and Decimal.unscaled are bounded on the wire only by MaxMessageSize, and a Decimal at exactly the scale bound renders to one digit more than the digit bound, the leading zero included. A producer of PXF text (a marshaller, or a binder rendering a (pxf.default) carrier) that meets such a value MUST fail with an error rather than truncate, round or change notation; the wire value itself remains valid and MUST be accepted by a PB decoder.
...
Per RFC 7942, this section records known implementations; it is not a
conformance statement and will be removed before any RFC publication.

* **Reference compiler** — the `trendvidia/protocompile` fork (Go)
  implements the `-01` grammar (contextual keywords, `type` /
  `function` / `annotation` declarations, annotation use sites), the
  intermediate representation, cross-file linking, and descriptor
  lowering to the carrier extensions of {{lowering}}, including the
  embedded source map. Argument-grammar conformance work (verbatim
  expression capture, named arguments, trailing field placement,
  function-reference extraction per {{function-references}}) is
  tracked and in progress.

* **Specification artifacts** — the canonical carrier schemas
  (`descriptor.proto`), the report shape (`report.proto`,
  {{report-wire-shape}}), the engine configuration
  (`config.proto`, {{engine-config}}), the framework annotation
  library (`annotations.proto`), and a conformance fixture corpus are
  published in the protowire source repository
  ({{PROTOWIRE-RFC-001}}).

* **Validation engine, language server, and code generation** — not
  started at the time of this revision; tracked in the protowire
  issue tracker.

# Open Issues {#open-issues}

The design questions raised in `-00` and during this revision's
drafting have been resolved and are specified in-document:
engine configuration ({{engine-config}}), well-known-type binding
({{wkt-binding}}), recursion depth limits ({{recursion-depth}}),
streaming RPC validation ({{streaming-validation}}), and the
validation-report wire shape ({{report-wire-shape}}).

The following items are formally deferred to a future revision and
tracked in {{PROTOWIRE-RFC-001}} and the protowire issue tracker:

* **Container-shaped type aliases.** Allowing `repeated` and
  `map<K,V>` as `type-ref` bases, with element-vs-container binding
  semantics.

* **Stream-level validation invariants.** Aggregate rules across a
  stream's messages and ordering constraints
  ({{streaming-validation}} is per-message only).

* **Sensitivity-class taxonomy.** An additive `class:` parameter on
  `@sensitive` ({{sensitivity}}) distinguishing e.g. secret / PII /
  confidential.

* **Schema-level encryption annotations.** A key-reference annotation
  (e.g. `@encrypted(key_ref)`); in this revision the schema stays
  orthogonal to key management.
