...
- **A formatter keeps the quotes on a string map key spelled like a keyword or an integer** (issue [#306](https://github.com/trendvidia/protowire/issues/306)). Since #284 a bare `true` / `false` is a bool key, so `"true": "v"` on a `map<string, V>` and `true: "v"` are different documents — and every formatter in the family wrote the second for the first, because it dropped the quotes from any identifier-shaped key. `pxf fmt` turned a document that binds into one that does not. Draft `-01` § Entries and Keys now states the canonical spelling of a string key: unquoted iff identifier-safe and not a value keyword, quoted otherwise, with `"true"`, `"false"`, `"null"` and integer-shaped keys always quoted — the rule § Encoding and Canonical Form already gave keyed entry names, and the rule the reference marshaller has always followed. `testdata/map-keys/fmt-keyword-keys` pins it as a fmt fixed point (schema `mapkeys.v1.Labels`, added to `bool-keys.proto`); the `cmd/pxf` assertion lands with the protowire-go release that carries the formatter fix (protowire-go#119). Decided in-change as option 1 of the issue: no AST change, one canonicalization rule.
