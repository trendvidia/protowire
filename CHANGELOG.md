...
- **§6.2 binds `this` the way §5.4 allows, and the IETF draft carries the expression language at all** (issue [#324](https://github.com/trendvidia/protowire/issues/324), decisions A and B). RFC-001 §6.2 rule 3 called `this.type_url == "…"` the canonical `Any` refinement and rule 4 let field access "follow the engine's proto integration"; §5.4, resolved later on #282, forbids field selection, and protocompile v0.33.0 rejects both. Rules 3 and 4 now say a rule over a message's fields — `type_url` included — is a declared `function` taking the message (decision A), and a sixth rule says an enum `this` binds to the value's number and a rule compares integers, since §5.4 has no enum-value reference and the reference engine never resolved one (decision B, measured on protocheck; a name-resolved spelling is a later-minor candidate). Draft `-01` had never received §5.4: its Engine Expressions section still said "opaque text for the engine to compile", an unresolved call was "presumed an engine builtin", rule 4 let rules "reference fields by name", and the engine configuration still selected a `cel` default. The draft gains an Expression Language subsection (grammar, the six builtins, typing, what the language omits), its function-reference and `expression`-parameter text follow, its binding rules match §6.2's six, and `EngineConfig.engine` is stated reserved as §9.4 has said since #282. Fixtures 09 and 14 cite the rules instead of the open decision.

- **The schema-extension corpus parses under §5.4, and the pinned compiler enforces it** (issue [#324](https://github.com/trendvidia/protowire/issues/324)). protocompile v0.33.0 checks every `expression` argument against RFC-001 §5.4 at compile time (trendvidia/protocompile#211, the follow-up #283 filed), and on that compiler three positive fixtures failed at five sites: `duration("60s")` and `this.type_url` in `09_wkt_refinements`, `split(…)[0]` in `12_expression_args`, `OrderStatus.SHIPPED` and `startsWith` in `14_refinement_kinds`. #283 had stated that every fixture parsed under the grammar; that was inferred, since no compiler enforced it and the harness's spec-ahead-of-toolchain pin covers only the invalid fixtures. The pin moves v0.32.0 → [v0.33.1](https://github.com/trendvidia/protocompile/releases/tag/v0.33.1); the five sites conform to §5.4 as written — `starts_with`; a declared `function` for the duration bound, as §6.2 rule 2 already says; a declared `function` taking the `Any` for the `type_url` check; a §5.4-valid nested capture that still exercises the §5.1 delimiter scan; enum values by number — and the fixture headers and README rows say §5.4 rather than "presumed engine builtin". `cmd/pxf`'s `checkKnownGaps` empties: `invalid/unresolved_expression_call.proto` and `invalid/expression_syntax.proto` are rejected now, the test flip the harness was built for. Two RFC-text questions the compiler exposed stay open on #324: §6.2 rules 3 and 4 still describe field selection, which §5.4 forbids (decision A), and §5.4 has no enum-value reference, so a refinement names the number — measured, the reference engine binds an enum `this` to its number and never resolved a qualified name (decision B).

- **HARDENING and draft `-01` say that a big number can be valid on the wire and have no PXF literal, and that this is the rule, not a corner** (issue [#310](https://github.com/trendvidia/protowire/issues/310)). `MaxNumericLiteralDigits` does two jobs — a work bound on `Decimal.scale` for the PB decoder, a length bound on any text literal — and the two disagree by one at the scale bound: a positive scale of *n* renders *n*+1 positional digits, so every `Decimal` at scale ±4096, which the wire MUST accept, renders past the cap and the reference marshaller refuses it (protowire-go's `TestDecimalScaleIsBoundedOnMarshal`). Filed as a boundary one-off with three options; measured, it is the general state: `BigInt.abs` and `Decimal.unscaled` are bounded on the wire only by `MaxMessageSize`, so a 5000-digit `BigInt` or a `Decimal` with 5000 unscaled digits is accepted over PB and has no literal either, exactly as the text already said of `BigFloat`. Decided as option 1: the text states that the bounds are independent, that a wire-valid big number may have no spelling, and that a producer of text meeting one MUST fail naming the digit count rather than truncate, round or change notation; the `pb/decimal-scale-4096` corpus row stays `accept`. Not option 2 (scale cap at 4095: makes the boundary pair renderable, leaves every other wire-only value where it is, and moves a MUST-level acceptance in every port) nor option 3 or exponent-form Decimal literals (grammar changes for the same non-result). The reference's pin gains a scale-0 Decimal past the digit cap so the general rule has a test, not just the boundary (protowire-go#126).

- **A dotted string map key is identifier-safe, and the ports write it bare** (issue [#313](https://github.com/trendvidia/protowire/issues/313)). Draft `-01` defines identifier-safe, for map keys and keyed entry names alike, as matching the *identifier* production — whose *ident-part* admits `.` — and not being a value keyword; the text already calls `user.name { }` a legal unquoted entry name. Every port's marshaller and formatter applied a narrower alphabet, `[A-Za-z0-9_]`, to string map keys only, so `a.b` was marshalled `"a.b":` and a quoted `"a.b"` kept its quotes through `fmt`, while the same ports unquoted dotted entry names. Decided as (a): the ports widen to the identifier production and the text stays as written — the grammar already admits `a.b:` as input and every port decodes it, so excluding dots from the formatter's test would have left two canonical spellings for one key. Keys that fail *ident-start* (`".e"`, `"1.5"`) stay quoted. A third fmt pair, `testdata/map-keys/fmt-dotted-keys`, pins both directions; `cmd/pxf` asserts it on protowire-go [v1.8.0](https://github.com/trendvidia/protowire-go/releases/tag/v1.8.0), the release carrying the reference's change (protowire-go#125); `go.mod` moves v1.7.0 → v1.8.0, and with it the pinned `trendvidia/protocompile` v0.32.0 → v0.33.1, which v1.8.0 requires — the rest of the suite is unchanged by either bump. Port PRs and issues: protowire-go#125, protowire-rust#49, protowire-typescript#44, protowire-java#83, protowire-cpp#36, protowire-csharp#36, protowire-swift#19, each sequenced after that port's #306 leg.

- **A formatter keeps the spelling of a map key where changing it would change what the key denotes** (issue [#306](https://github.com/trendvidia/protowire/issues/306)). Since #284 a bare `true` / `false` is a bool key, so `"true": "v"` on a `map<string, V>` and `true: "v"` are different documents — and every formatter in the family wrote the second for the first, because it dropped the quotes from any identifier-shaped key. `pxf fmt` turned a document that binds into one that does not. Draft `-01` § Entries and Keys now states the canonical spelling of a map key: a bare key stays bare; a quoted key is unquoted only when it is identifier-safe and not a value keyword, so `"true"`, `"false"`, `"null"` and a quoted integer stay quoted. Formatters retain whether a key was quoted, as they already do for keyed entry names (decided as option 2 on #306: the alternative, always quoting keyword-shaped keys, would have rewritten every bool-keyed map's canonical form). One existing output moves: a bare integer key, which every formatter quoted (`404:` → `"404":`), stays bare, as the marshaller has always written it. `testdata/map-keys/fmt-keyword-keys` and `fmt-bare-keys` pin both directions (schema `mapkeys.v1.Labels` added to `bool-keys.proto`); the `cmd/pxf` assertions land with the protowire-go release that carries the formatter fix (protowire-go#123).

- **The corpus's digit-cap row proves the cap, and `Decimal.scale` has rows** (issue [#279](https://github.com/trendvidia/protowire/issues/279)). `pxf/long-numeric.pxf` decoded against an `int64` field, so a port with no digit cap still rejected its 5000 digits as out of range and met the verdict for a reason unrelated to the limit. `adversarial.proto` gains `BigNumHolder` with `pxf.BigInt`, `pxf.Decimal` and `pxf.BigFloat` fields; the row moves to the `pxf.BigInt` field, where only `MaxNumericLiteralDigits` can reject it, and a 4096-digit sibling sits at the cap and must be accepted. Three `pb/decimal-*` rows bound `Decimal.scale` at ±`MaxNumericLiteralDigits` on the wire — `2³¹−1` and `−2³¹` refused before `10^scale` is materialised, `4096` accepted — per the § Mandatory limits text #278 amended. The `pb` rows skip every port but the reference until its `check-decode` links the new message (protowire-go#117 for the reference).

- **The HARDENING conformance corpus runs in CI** (issue [#303](https://github.com/trendvidia/protowire/issues/303)). `cross-port-security-check.yml` runs `scripts/cross_security_check.sh` daily and on the paths that change the corpus, for the same four ports the envelope gate builds; it read the ports only when someone ran it by hand, which is how nine failing rows on the C++ port went unnoticed (protowire-cpp#26). C++ is skipped in the workflow until that lands, so a known-red port does not hide a new failure elsewhere.

- **`cmd/pxf` asserts the duration diagnostics and drives the bool map-key corpus** (issues [#261](https://github.com/trendvidia/protowire/issues/261), [#286](https://github.com/trendvidia/protowire/issues/286)). Both waited on a protowire-go release: `go.mod` moves `protowire-go` v1.5.1 → [v1.6.0](https://github.com/trendvidia/protowire-go/releases/tag/v1.6.0), which carries the lexer diagnostic reaching the caller (protowire-go#77), the bool map-key spelling fix and the keyword production (protowire-go#93, #109), the `pb` map-entry layout and plain-varint big-number fields (protowire-go#105, #92), and the mandatory size limits (protowire-go#112) — so `pxf validate` and `encode` now refuse a document over 64 MiB, as HARDENING requires. `TestDuration_Rejects` asserts that each reject fixture names the malformed duration (`invalid duration: 5seconds`, `invalid duration: 5sx`, `got duration ("-30m")`) rather than merely failing; `cmd/pxf/map_keys_test.go` runs the three MUST-bind documents through `validate` and `encode`, reading the bytes back to exactly the keys `true` and `false`, and the thirteen `invalid/` documents through `validate`, each rejected naming its key. The map-keys README's reference wording follows v1.6.0, which lists the keyword spelling among the admitted ones.

- **The corpus pins both sides of `MaxNestingDepth`, and HARDENING says where the counter starts** (issue [#301](https://github.com/trendvidia/protowire/issues/301)). § Recursion defined the counter as "incremented on every recursive descent" and rejected "when it exceeds `MaxNestingDepth`" — a root at depth 0 — but the corpus measured nothing between 10 and 200 levels, so the reference's PXF text parser accepted the hundredth level while its PXF decoder and its `pb` decoder counted the root as 1 and refused it (protowire-go#111). The text now says it in one sentence, for `{` and `[` in PXF and for submessages, groups and map entries in PB alike, and six generated rows sit on both sides of the edge; `Tree` gains a `repeated children` so a document can descend through lists and blocks together. Every port is measured on them.
...
- **The conformance corpus proves `MaxMessageSize`, `MaxBytesLiteralLength` and `MaxRepeatedCount` with small fixtures under per-call limit overrides** (issue [#299](https://github.com/trendvidia/protowire/issues/299)). HARDENING.md § Mandatory limits made all three mandatory; nothing measured them, and measured on 2026-09-07 no port enforced any — 14 of 14 verdicts failed on two 65 MiB probes across the seven ports the harness runs. A manifest entry may now carry `limits`, which `scripts/cross_security_check.sh` passes as `check-decode --limit NAME=VALUE`, so six new fixtures (2 KiB inputs, a 192-byte `b"…"` literal, 16-element lists and a 16-entry SBE group) prove each cap twice: rejected under a lowered limit, accepted under one above their size. `adversarial.proto` gains `ListHolder` for the repeated-field rows; HARDENING states that a stream's frame cap and `MaxMessageSize` coexist. Every port but the reference is in the new rows' `skip` until its `check-decode` takes the flag; STABILITY.md's v1.13 section records the narrowing. Port issues: protowire-go#112 (fixed in protowire-go#115), cpp#25, java#79, rust#33, swift#16, csharp#32, dart#23.

### Changed

- **A `pb` map entry always carries its key and its value, zero-valued or not** (issue [#295](https://github.com/trendvidia/protowire/issues/295); decided on [trendvidia/protowire-go#105](https://github.com/trendvidia/protowire-go/issues/105)). Measured across eight ports on 2026-09-07 with one message, the schema-free codecs wrote three layouts for an entry with a zero key or value — Go, C++, Java and Rust omitted the field; C# and Swift wrote it; TypeScript skipped a zero key and wrote a zero value — and every reader accepted all three, so the split was lossless and invisible. The family converges on the layout protobuf-go, protoc and C++ protobuf write, both fields always, so conformance has an external oracle rather than the reference's own history. `STABILITY.md`'s v1.13 section records why promise 2's bytes may move and that they move once, together with the reference's plain-varint `Decimal.scale` / `BigFloat.exponent` (protowire-go#96). `scripts/cross_envelope_check.sh` gains a golden-checked wire-vector leg, `dump-envelope --vector zero-map-entry`, with [`testdata/envelope/`](testdata/envelope/) holding the vector, its text form and its expected bytes from two oracles; every port is declared not-implemented in `VECTOR_NOT_IMPLEMENTED` until its PR lands. Port issues: protowire-go#105, protowire-cpp#24, protowire-typescript#42, protowire-rust#32, protowire-java#78 (after protowire-java#77, which found the module zigzag-encodes every integer), protowire-csharp#31, protowire-swift#15, protowire-dart#22.

- **RFC-001 §8.1 and draft `-01` state which `AnnotationArg` / `LiteralValue` member a numeric literal takes, and the corpus asserts it** (issue [#262](https://github.com/trendvidia/protowire/issues/262)). The reference compiler had answered the question three different ways in five releases, each as a bug fix, because there was no spec text to amend; #264 wrote the rule into `descriptor.proto`'s comments, and this puts it where the RFC's lowering rules live, normatively: the member is decided by type — the declared parameter type, or for `any` the annotated element's type, with the literal's own spelling standing only where there is no element — and a literal the type cannot hold is a compile error, never a different member or a wrap. `10_literal_args.proto` gains one field per cell of the rule (`Numeric`, `SpelledFloat`, `SpelledInt`), `11_literal_carrier_golden.textproto` records the lowered member for each, and `cmd/pxf/literal_carrier_test.go` compares the golden with what the reference lowers, entry by entry — the golden was previously only syntax-checked, so the corpus would have passed under every historical rule. Two MUST-NOT-COMPILE fixtures pin the error cases. The pinned `trendvidia/protocompile` moves v0.25.0 → v0.32.0, the first version that lowers by the rule; the rest of the suite is unchanged by the bump.
...
- **`@validate` expressions are one normative language** (RFC-001 §5.4, issue #282). The ratified text left an expression argument's body to "the engine" and let §9.4 pick `cel` / `starlark` / `go`, `cel` by default. Measured against what evaluates the carrier: one engine exists (protocheck) and it speaks none of those — its own small dialect — while protovalidate never sees `@validate` at all (the annotation lowers only to the `1327` carrier, never to `buf.validate.field`). With no defined language a misspelled builtin compiled clean and failed at descriptor load, and a shared `type` alias carried no statement of what its rule was written in. §5.4 now states the grammar, the six builtins (`size`, `starts_with`, `ends_with`, `contains`, `matches` in method and function form, plus `now()`, which §6.2 already promised), and the typing and evaluation-error rules; §5.1 and §8.1 make an unresolved call and a syntax error compile errors instead of presumed builtins; `EngineConfig.engine` is reserved and its `cel` default retired (§9.4, `config.proto` comment, fixture 08); §3, §6.2, §7's engine example, §9.1, §10 and Appendix A/C are reconciled. New conformance fixtures: `24_expression_language.proto`, `invalid/unresolved_expression_call.proto`, `invalid/expression_syntax.proto`. Every existing fixture and the §5.3 worked example already parse under the grammar. Follow-ups: protocompile parses the fragment against §5.4 and re-vendors the fixtures (trendvidia/protocompile#211 — until then `cmd/pxf`'s `checkKnownGaps` names it for the two new invalid fixtures); protocheck adds `now()` and temporal ordering (trendvidia/protocheck#64).

- **`MaxNumericLiteralDigits` also bounds the magnitude of `Decimal.scale` on the PB wire, and a big-number value a port cannot represent is an error** (issue [#278](https://github.com/trendvidia/protowire/issues/278)). Draft `-01` § Mandatory Limits and [`docs/HARDENING.md`](docs/HARDENING.md) § Mandatory limits: a decoder rejects a `pxf.Decimal` whose scale magnitude exceeds 4096 *before* materialising `10^scale` (a scale is a digit count, so the existing limit names the right quantity and no new row is added); `BigFloat.exponent` has no limit and costs no work at decode, the cost of rendering it in decimal being the consumer's and bounded by the same limit where the result is a PXF literal; and a literal or message whose value the arbitrary-precision type cannot hold is an error, never an infinity or a truncated value. This strengthens a requirement, so under HARDENING's versioning rule this entry is the one-minor-version notice. Measured on the Go reference, decoding a five-byte Decimal with `scale = 2³¹−1` did not return in 5 s ([protowire-go#95](https://github.com/trendvidia/protowire-go/issues/95)); the corpus entry that proves the bound is [#279](https://github.com/trendvidia/protowire/issues/279).
...
- **`scripts/cross_envelope_check.sh` now proves the extension numbers, not only the envelope** (issue [#244](https://github.com/trendvidia/protowire/issues/244), the done-when it had left open). The envelope leg carries no annotations, so nothing in the gate could tell whether a port read `(pxf.default)` from `1315` or from the retired `50001` — and a port's own suite cannot either, because it compiles the port's own vendored `annotations.proto`, so reader and copy agree with each other whatever number they hold. Both near-misses recorded on #244 (TypeScript's inline `50_200`, C#'s base64 descriptor blobs) are that class. Two legs are added: every descriptor-driven port's `dump-envelope` gains `--pb FDS MESSAGE DOC` and `--sbe FDS MESSAGE DOC`, decoding a PXF document against a message in a descriptor set the script supplies and printing the bytes (exit 1 + `reject: …` when the schema rejects the document), and the script compares them with a checked-in golden rather than with each other, since ports that all misread the same number would agree. New fixtures: [`testdata/annotations/`](testdata/annotations/) (`settings.proto` with `(pxf.required)` and three `(pxf.default)`s, an `ok.pxf` whose absent fields must take their defaults, a `missing-required.pxf` that must be rejected, and the golden) and [`testdata/sbe-bench.pxf`](testdata/sbe-bench.pxf) (the canonical `bench.v1.Order` as text, golden in `sbe-bench.expected.hex`, header = blockLength 42, templateId 1, schemaId 1, version 0). Negative control: against a descriptor compiled with the retired numbers, Go, C++, TypeScript and Java all emit `ok.pxf` without its defaults and accept `missing-required.pxf`; against the checked-in sets all five ports match the golden byte for byte. Port PRs: [protowire-go#97](https://github.com/trendvidia/protowire-go/pull/97), [protowire-typescript#36](https://github.com/trendvidia/protowire-typescript/pull/36), [protowire-rust#30](https://github.com/trendvidia/protowire-rust/pull/30), [protowire-java#59](https://github.com/trendvidia/protowire-java/pull/59), [protowire-cpp#22](https://github.com/trendvidia/protowire-cpp/pull/22). Swift and Dart take part with a generated message type and the ids read from the descriptor set, which needed descriptor-driven SBE template building in each — `SBE.templates(from:)` ([protowire-swift#12](https://github.com/trendvidia/protowire-swift/pull/12)) and `Codec.registerFromDescriptorSet` ([protowire-dart#16](https://github.com/trendvidia/protowire-dart/pull/16), which also adds the envelope dumper Dart never had); their pb leg is a *declared* omission (exit 3, `not-implemented`), because neither PXF decoder implements `(pxf.required)`/`(pxf.default)` ([protowire-swift#11](https://github.com/trendvidia/protowire-swift/issues/11), [protowire-dart#14](https://github.com/trendvidia/protowire-dart/issues/14)) — the script fails on an undeclared one, the distinction #253 drew for the drift check. C# and the Java/Android lite dumpers are `SKIP` lines citing [protowire-csharp#26](https://github.com/trendvidia/protowire-csharp/issues/26) and [protowire-java#58](https://github.com/trendvidia/protowire-java/issues/58). Rust resolves the SBE options by extension name from the set's own declarations rather than by number, which is self-consistent and is recorded in [`STABILITY.md`](STABILITY.md) § v1.12 as not a promise.
...
- **The duration grammar admits the sign every port already writes** (issue [#234](https://github.com/trendvidia/protowire/issues/234)). `google.protobuf.Duration` is signed, and every encoder in the family emits a negative Duration with a leading `-` because each mirrors `time.Duration.String()` — Go `-1.5s` (pinned in `TestNegativeDuration` / `TestFullRoundTripNegativeDuration`), Rust `format_go_duration`, TypeScript `formatGoDuration` (pinned in `duration.test.ts`), Java, C++ — and every parser accepts it. Draft `-01` §3.3's production, `duration = 1*duration-segment`, admitted no sign, so a conformant encoder emitted a token the grammar could not derive and a strictly grammar-driven decoder would have rejected the family's own output for every negative Duration. The production is now `duration = ["-"] 1*duration-segment`, and §"Timestamps and Durations" says what that means: one leading `-` before the first segment applying to the whole literal (`-1h30m` is minus ninety minutes), never a sign per segment or a `+`; a decoder binds a fractional negative to seconds and nanos of the same sign, as `google.protobuf.Duration` requires; `-0s` is zero. **No port changes**: the text now derives what the ports have always emitted and accepted. Additive at a minor per [`STABILITY.md`](STABILITY.md).
  ...
  The same section's disambiguation prose said an identifier "whose initial characters happen to match a duration prefix (for example `5seconds`) is tokenized as an identifier". No port does that, and the grammar cannot: `identifier` never begins with a DIGIT, and the very next bullet sends a leading DIGIT down the numeric branch. Every port read (Go, Rust, Java, TypeScript, C++, C#, Dart, Swift) scans `5seconds` as a duration candidate, fails validation, and rejects it — Rust pins `5sx`, Go pins `5min` and `1.5min`. The sentence now says that: a DIGIT-led token that continues past a time-unit into further ALPHA is an invalid duration and MUST be rejected as such, never an identifier. `docs/grammar.ebnf` and the railroad diagram carry the same one-token change.

  **What stays open, on purpose:** whether a leading `+` is admitted. It is not in the grammar and no encoder writes it, but the ports differ on lenience — the reference port rejects `+30s` at the lexer (`+` begins nothing but `+inf`, §3.8), while several others skip a `+` inside their duration validators — so the text says only that decoders accepting it are lenient rather than conformant, and the fixture corpus deliberately carries no `+` case: one either way would break half the family before the decision is taken. #234 records the recommendation (`-` only).

  New cross-port fixtures under [`testdata/duration/`](testdata/duration/) (`duration.proto`, package `duration.v1`), in the shape of `testdata/keyed/`: four accept fixtures with the `(seconds, nanos)` each binds to, one fmt fixed-point pair (`-1.5s` re-encodes as `-1.5s`), and three reject fixtures (`1h-30m`, `5seconds`, `5sx`). `cmd/pxf/duration_test.go` runs all of them through the reference implementation, which needed **protowire-go v1.4.1** (`go.mod` bump from v1.3.2): v1.3.2 predates the fractional-duration lexer fix ([protowire-go#75](https://github.com/trendvidia/protowire-go/issues/75)) and read `-1.5s` as `-1.5` followed by an identifier `s`. One diagnostic gap surfaced on the way and is filed, not papered over: the reference lexer produces `ILLEGAL "invalid duration: 5seconds"` but the parser reports `expected '{' for message field` ([protowire-go#77](https://github.com/trendvidia/protowire-go/issues/77)); the reject fixtures assert rejection, which is what the draft requires, and will assert the message once that lands.

## [1.11.0] – 2026-08-09

`pxf.default` gets the placement rules it never had, and the bind-time checks get a scope. The draft specified what the annotation's *value* is and said nothing about where it may be set, so five implementations each invented an answer and one of them invented `pb` bytes — measured across the family before a word of text was written. Draft `-01` §annotation-extensions now states where a single literal may sit ("Default Placement") and what both `pxf.default` and `pxf.required` mean on a member of a oneof ("Oneof Members"); §schema-constraints states, for the first time, which files a bind-time check covers at all ("Scope of Bind-Time Checks") — the bound descriptor's declaring file **and its transitive imports**, since a descriptor spans more than one file and every port had quietly assumed the narrowest reading. No grammar, wire-format, envelope, report, or descriptor change: PXF, `pb`, SBE, envelope and report outputs are byte-identical to v1.10.0 for every schema and document that binds under both — **except inside a oneof**, where a member's `pxf.default` no longer overwrites the arm the document chose. That is a deliberate behaviour change and the one place this release is not byte-identical. Accepted schema input narrows in four places, every one of them a placement no implementation could ever honor, so the alternative was carrying a dead annotation in silence; see [`STABILITY.md`](STABILITY.md) for each narrowing and why it is taken at a minor. Ports: trendvidia/protowire-go v1.4.0 implements all three sections; trendvidia/protowire-rust#25, trendvidia/protowire-java#54 and trendvidia/protowire-typescript#33 track the rest of the family, none of which had implemented the placement checks at all.

### Added

- **`pxf.default` gains a placement constraint** (issue [#223](https://github.com/trendvidia/protowire/issues/223)). The draft specified the annotation's *value* — "a PXF literal, parsed by the same rules as a value in any entry" — and said nothing about where it may be set, while its sibling `pxf.key` has carried an explicit placement rule and a bind-time MUST-reject list since v1.2. Five implementations each filled the gap differently, and one of them filled it with `pb` bytes: measured across the family, protowire-go panicked inside `protoreflect.Message.Set` on a `repeated` placement, protowire-java threw `ClassCastException` out of the decoder, protowire-typescript reported an error, chameleon silently ignored the annotation, and **protowire-rust silently applied the single literal as a one-element list** — so for one schema and one document it emitted repeated-field bytes no other port emitted, which is what the [`STABILITY.md`](STABILITY.md) promise 2 byte-equivalence commitment forbids. `scripts/cross_envelope_check.sh` does not cover the shape.

  Draft `-01` §annotation-extensions gains a "Default Placement" subsection modelled on §keyed-schema-placement: because the annotation carries exactly one literal, it is valid only on singular scalar fields, enum fields, and the message types a literal can denote (`google.protobuf.Timestamp`, `google.protobuf.Duration`, the nine `*Value` wrappers, `pxf.BigInt`, `pxf.Decimal`, `pxf.BigFloat`), and tools that bind a descriptor for PXF use MUST reject a `repeated` field, a `map<K,V>` field, a group, or any other message type **at bind time, independently of what any document contains**. Deferring the diagnostic to the first document that leaves the field absent — the decode-time reading — was rejected because a schema whose annotated field happens to always be present would then carry a meaningless annotation indefinitely without one. Implementations MUST NOT invent a semantics for a rejected placement; the one-element-list reading is named and forbidden, as it is also the option trendvidia/chameleon#131 rejected on its own merits.

  The constraint is on placement, not on the literal: `"abc"` on an `int32` field stays a decode-time error, because placement is determinable from the descriptor alone whereas checking the literal would oblige every binding tool to implement the value parser. RFC-001 §5.2 records that `@default(value)` inherits the rule — it is a constraint on the annotation, not on the surface that writes it — and notes the one place `@default` and `@validate` diverge on collections: §6.4's per-element rule gives `@validate` a meaning on `repeated`/`map<K,V>` that `@default` has no grammar to express.

  **This narrows accepted schema input at a minor version**, which the post-v1.0 additive-only rule otherwise reserves for a major bump. Same reasoning as the v1.10.0 `@http` bullet and as the §schema-constraints reserved-name rule it mirrors: every newly-rejected placement is one no implementation could ever honor, so the alternative is shipping a dead annotation in silence, and the diagnostic names the offending field by fully-qualified name. Migration is deleting the annotation or moving it to a singular field. Schema input only — a schema that binds under this revision decodes every document it decoded before, to the same bytes.

  Port bugs filed against the pre-constraint behaviour, both independent of this rule: trendvidia/protowire-rust#23 (silent one-element list) and trendvidia/protowire-java#52 (`ClassCastException`). The protowire-go side is trendvidia/protowire-go#68.

- **`pxf.default` and `pxf.required` gain oneof-member semantics** (issue [#226](https://github.com/trendvidia/protowire/issues/226)). The placement rule above enumerates what `pxf.default` may sit on by cardinality and type; a oneof member is a singular scalar, so it bound clean — and then decoded destructively. Setting any member of a oneof clears the others, so the per-field reading of "absent" both annotations are defined against does not hold inside one: a member is absent precisely when a sibling was chosen. Measured before the change, in **all four ports that implement the annotations** — protowire-go, protowire-java, protowire-rust, protowire-typescript, byte-for-byte identical behaviour, so unlike #223 this was one design bug inherited uniformly rather than five answers to an unasked question:

  ```proto
  oneof choice { string a = 1; string b = 2 [(pxf.default) = "bbb"]; }
  ```

  `a = "written"` decoded to `b = "bbb"`, case `b` — **the value the document wrote was gone**, not shadowed but cleared. One annotated member was enough; capping the count would not have helped. With two annotated members an empty document's result depended on declaration order. And `(pxf.required)` on a member rejected `b = "written"` with `required field "a" is absent`, making every arm but the annotated one undecodable.

  Draft `-01` §annotation-extensions gains an "Oneof Members" subsection. `pxf.default` MAY sit on a oneof member, applying **only when no member of the oneof is present** (a member bound to `null` counting as present, consistent with the existing null-suppresses-default rule), and **at most one member of any one oneof may carry it** — with two, some default must win, and deciding by declaration order or field number attaches meaning to a detail authors are free to change, so the schema is rejected instead. `pxf.required` **MUST NOT** sit on a oneof member: its only coherent reading, "the oneof must be set", is a property of the oneof, and this revision defines no annotation at that scope — forbidding the member placement keeps a future oneof-scoped annotation additive rather than a reinterpretation.

  **This is the one place v1.11 changes decoded output**, not just what binds; see [`STABILITY.md`](STABILITY.md). Forbidding `pxf.default` on oneof members outright was considered and rejected — "if the document picks no arm, use this one" is a real capability and the bug was in the presence test, not the idea. Port side: trendvidia/protowire-go#72 carries the Go implementation and the measurement; the same runtime bug reproduces identically in the other three.

- **Bind-time checks are scoped to the import closure** (issue [#228](https://github.com/trendvidia/protowire/issues/228)). Both rules above, and §schema-constraints' reserved-name rule before them, say a binding tool MUST reject "a schema" — and a descriptor spans more than one file. Every port read that as the file declaring the bound message; nothing in the draft said so, and the reading does not satisfy the conformance text it appears under. Measured on protowire-go at 0f96d76 with `repeated string tags = 1 [(pxf.default) = "ignored"]` declared in an imported `.proto`:

  | Call | Result |
  | --- | --- |
  | `ValidateDescriptor(Root)` | `[]` — clean |
  | `ValidateDescriptor(Inner)` | 1 × placement violation |
  | decode `inner { }` through `Root` | decode-time error |
  | decode `` (empty) through `Root` | **no error, no diagnostic** |

  The last row is exactly what "Default Placement"'s first conformance bullet was written to forbid — a schema carrying a meaningless annotation indefinitely with no diagnostic — reached through an import instead of through a document that always populates the field. Runtime-probed in protowire-rust and protowire-typescript, source-read in protowire-java: all four scope to the declaring file, and three of them have not implemented the placement checks at all yet (`ViolationKind` is still `{Field, Oneof, EnumValue}`), so this settles the scope before there is anything to migrate.

  Draft `-01` §schema-constraints gains a "Scope of Bind-Time Checks" subsection, stated once and referenced from §keyed-schema-placement, §default-schema-placement and §oneof-annotation-placement rather than restated in each. The scope is the file declaring the bound descriptor **together with its transitive imports**; a file reached twice is checked once, and a violation is attributed to the file that declares it rather than the file that was bound. The third candidate — only the message types a decoder can reach by following message-typed fields — was rejected as not evaluable at bind time: a `google.protobuf.Any` payload type is named by the document and resolved through a type resolver at decode time, so the reachable set is not a function of the descriptor.

  **This narrows accepted schema input**, on the same reasoning as the bullets above but with the widest blast radius in the release: one bad placement used to make one file non-bindable and now makes every transitively-importing file non-bindable. That is the intent — a widely-imported file is where a dead annotation does the most damage — and it is called out in [`STABILITY.md`](STABILITY.md) rather than left to be discovered. Non-normative implementation note in the same subsection: memoize per file descriptor. Benchmarked on protowire-go, an unmemoized closure walk costs ~10.4 µs against a ~5.6 µs decode, while the memoized walk costs ~13 ns against the ~650–920 ns *single-file* walk it replaces — every annotated schema's closure reaches `google/protobuf/descriptor.proto` through `pxf/annotations.proto`, 54 elements no PXF document can name. Port side: trendvidia/protowire-go#71.
...
- **The renderer accepts every path-template shape the compiler does** (issue [#217](https://github.com/trendvidia/protowire/issues/217)). The two ends disagreed about what a `{…}` segment may contain: the compiler resolves a **dotted field path** from the request message's top level and accepts the `HttpRule` sub-path form `{name=shelves/*}`, while §5.2 and `pxf openapi` said same-named-top-level only. A schema using either form compiled, bound, and then failed to document — compile-and-bind-then-refuse, which is the #213 trap wearing a louder coat. §5.2 is widened to the compiler's rule (dotted paths, no component descending through a scalar, no `repeated` component) and the renderer implements it: a dotted variable becomes an OpenAPI parameter of the same name, and a sub-path constraint is dropped from the **path key** only — OpenAPI's template grammar has no `{name=pattern}` form — leaving `/named/{name}`. A nested binding removes only the leaf it names from the remaining-field binding ([#218](https://github.com/trendvidia/protowire/issues/218)): its siblings still bind under dotted names, where an `HttpRule` consumer puts them — `shelf.display_name` in the query string beside a path-bound `{shelf.id}`, and in the body the container carried minus that leaf. Descending exposes a container's own message-typed members to the binding, so one with nothing bound below it is refused like any other; a binding that descends *through* a self-referential type is refused by name rather than flattened forever, counting the request message itself as the first step of the descent. Flattening dissolves the `$ref` a container's `@sensitive` marker sat on, so the §6.7 minima travel with it — a leaf lifted out of a sensitive container is marked wherever it lands, and an inlined container keeps its own `@description`, `x-validation` and §6.7 markers on the object that replaces the reference. Three edges of the agreement stay open and are stated in §5.2 rather than left to be discovered: a **message-typed leaf** (`{mid.inner}`, and equally a bare `{mid}`) still compiles and binds and then fails to render; **two bindings differing only in a sub-path constraint** normalise to one OpenAPI path key and are refused as a duplicate operation, which is the canonical `{name=shelves/*}` + `{name=shelves/*/books/*}` shape; and the **constraint reaches no reader**, since dropping it from the path key drops it from the document. The duplicate-operation diagnostic names both source paths and the key they collided on, because after normalisation the key it reports may appear in no schema. New conformance fixture `23_http_template_paths.proto`.
