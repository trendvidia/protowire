...
| `21_http_operation.proto` | The enriched `@http` operation surface (§5.2, issue #173): the v1.2.0 two-argument form alongside uses that set every added parameter — `summary` / `operation_id` as plain string args, `tags` / `security` as `Literal.list` string lists — plus a named subset after the positional routing pair, and a `{name}` path template. Additive-contract proof: the bare form must keep lowering to exactly the args it wrote. Also the dual-lowering contract (issue #213): every method MUST carry the standard `google.api.http` option (`72295728`) beside the `50400` carrier — `get: "/orders"`, `get: "/orders/{order_id}"`, `post: "/orders"` with `body: "*"` — with no `google/api/annotations.proto` import added to the image. Compile + lowering only; operation *metadata* carries no validation semantics, so there is no golden |
