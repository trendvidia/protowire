...
├── 22_http_additional_bindings.proto        — repeated @http → additional_bindings (§5.2, issues #213/#215)
├── 23_http_template_paths.proto             — dotted + sub-path @http template segments (§5.2, issue #217)
...
| `09_wkt_refinements.proto` | `type` aliases on `Timestamp`/`Duration` (engine-native binding; `now()` builtin, a declared function for a duration bound) and `Any` (`type_url` refinement through a declared function, no auto-unpack) per §6.2 and §5.4 (issue #324) |
| `10_literal_args.proto` | Enum-value reference and homogeneous list literal as annotation arguments on `any`-typed params (§8.1) |
| `11_literal_carrier_golden.textproto` | Lowered `AnnotationList` with all three `Literal` kinds: resolved `EnumLiteral`, `Any` message literal, `ListLiteral` of `LiteralValue`s (§8.1, issue #64) — and the scalar member each numeric literal in `10_literal_args.proto` lowers to (§8.1 "Choosing the member", issue #262). Compared with the reference lowering by `cmd/pxf/literal_carrier_test.go` |
| `12_expression_args.proto` | Engine-expression arguments (§5.1 capture-then-classify, issue #91): balanced-delimiter capture with opaque string literals, named args after an expression arg, declared-function call extraction into `Expression.calls` vs. §5.4 builtins, which are never extracted (§8.1) |
...
| `14_refinement_kinds.proto` | Refinement over every value-shaped base kind (§6.3): enum (by value number — §5.4 has no enum-value reference, issue #324), wrapper (with a chained derived alias), and message bases; a field-level rule stacked atop the chain; an alias consumed as a repeated element type |
...
| `21_http_operation.proto` | The enriched `@http` operation surface (§5.2, issue #173): the v1.2.0 two-argument form alongside uses that set every added parameter — `summary` / `operation_id` as plain string args, `tags` / `security` as `Literal.list` string lists — plus a named subset after the positional routing pair, and a `{name}` path template. Additive-contract proof: the bare form must keep lowering to exactly the args it wrote. Also the dual-lowering contract (issue #213): every method MUST carry the standard `google.api.http` option (`72295728`) beside the `1327` carrier — `get: "/orders"`, `get: "/orders/{order_id}"`, `post: "/orders"` with `body: "*"` — with no `google/api/annotations.proto` import added to the image. Compile + lowering only; operation *metadata* carries no validation semantics, so there is no golden |
| `22_http_additional_bindings.proto` | A method carrying several `@http` use sites (§5.2, issues #213/#215) — the shape that expresses a versioned or aliased route. They lower to one carrier entry each in source order, and to one `google.api.HttpRule` whose first pattern is the rule and whose remainder are `additional_bindings` in the same order. Bindings after the first MUST name their own `operation_id`: the derived `<Service>_<Method>` is document-unique only while a method has one binding, and an index-derived id would rename a generated client method on a reorder. That obligation is the *renderer's* — the compiler carries the arguments either way, and a port rendering no REST surface never checks it. The coverage denominator counts methods, not bindings, so this file contributes one `@http` method. Compile + lowering only; no golden |
| `23_http_template_paths.proto` | The `@http` path-template shapes beyond a bare `{name}` (§5.2, issue #217): a dotted field path (`{shelf.id}`) resolving from the request message's top level, and a variable constrained by a sub-path pattern (`{name=shelves/*}`). Both reach the carrier and `google.api.http` verbatim, patterns included. A renderer normalises only what its own format cannot express — OpenAPI has no `{name=pattern}` form, so the path key keeps the variable alone, while a dotted variable carries through (OpenAPI parameter names admit dots). Pins that a nested binding removes only the leaf it names from the remaining-field binding — `shelf.display_name` still binds under its dotted name, where an `HttpRule` consumer puts it (issue #218) — while a container's own message-typed members with nothing bound below them are refused as they always were. The three edges where the two ends still disagree — a message-typed leaf, two bindings differing only in a constraint, and the constraint reaching the document at all — are named in the file header and stated in §5.2; they refuse at render time, so no positive fixture can carry them. Compile + lowering only; no golden |
| `24_expression_language.proto` | The normative expression language (§5.4, issue #282): every builtin in both its method and its function form (`this.size()` / `size(this)`, `starts_with`, `ends_with`, `contains`, `matches`), `now()` against a `Timestamp` field, every operator (`&&`, `||`, `!`, the six comparisons, `in`), nested parentheses, integer / float / string / boolean / list literals, a declared-function call, and a `type` alias rule propagated onto a `repeated` field. Compile + lowering only: every rule lowers to the `1327` carrier with `Expression.calls` naming exactly the declared-function calls (never a builtin), and no rule is a compile error; evaluation is pinned by the reference engine's own suite, so there is no golden |
| `invalid/` | Sixteen MUST-NOT-COMPILE fixtures — arity mismatch ("invalid signature"), positional-after-named, heterogeneous list, unknown literal field, untyped list element, map field in literal, container alias, cyclic alias, unbalanced capture, undeclared annotation, reserved sensitive class, and the five `@http` routing classes (unbound path template, repeated-field template, relative path, unbalanced template, empty method) — with the error-class manifest in `invalid/README.md` |
...
1. **`protocompile`** (reference v1.2 toolchain) parses and lowers the
   positive corpus to a `FileDescriptorSet` carrying the `1327`–`1331`
   extensions.
