...
| File | Purpose |
|---|---|
| `store.proto` | One schema exercising every arm of the §#080 mapping: mappable and unmappable `@validate` shapes, alias chains, §6.7 sensitivity minima, presence, derived error codes, and the §5.2 operation surface (path/query/body binding, defaulted and explicit operation metadata, and a method carrying two `@http` bindings — one operation each, the alias naming its own `operation_id`, issue #215). |
