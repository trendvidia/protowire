# `@table` adversarial fixtures

Reference fixtures for the `@table` directive (draft §3.4.4). Each
...
cross-port contract every port's `@table` implementation must
satisfy. The conformance harness wiring is deferred — see the
CHANGELOG entry for `@table` and `scripts/cross_security_check.sh`
